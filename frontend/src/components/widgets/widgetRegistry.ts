import type { WidgetTypeDefinition, WidgetTag } from '../../types';
...
/**
 * Widget tag metadata for display in the widget picker
 */
export const TAG_METADATA: Record<WidgetTag, { name: string; order: number }> = {
  structural: { name: 'Structural', order: 1 },
  decorative: { name: 'Decorative', order: 2 },
  gauge: { name: 'Gauges', order: 1 },
  status: { name: 'Status', order: 2 },
  media: { name: 'Media', order: 3 },
  overview: { name: 'Overview', order: 4 },
  feed: { name: 'Feeds', order: 1 },
  tracker: { name: 'Trackers', order: 2 },
  utility: { name: 'Utility', order: 3 },
  timing: { name: 'Timing', order: 1 },
  atmosphere: { name: 'Atmosphere', order: 2 },
};
...
export const WIDGET_TYPES: Record<string, WidgetTypeDefinition> = {
  ...
  countdown_timer: {
    ...
    name: 'Timers',
    description: 'Countdown and stopwatch timers for GM tracking',
    category: 'gm',
    tags: ['timing'],
    minWidth: 3,
    minHeight: 4,
